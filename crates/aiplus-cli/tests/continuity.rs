...
#[test]
fn doctor_reports_deferred_nl_role_status_for_opencode() {
    let temp = tempfile::tempdir().unwrap();
    let env = fake_env(temp.path());
    let project = temp.path().join("project");
    fs::create_dir_all(&project).unwrap();
    ...
    assert_nl_role_trigger_catalog(&agents);
    let doctor = stdout(&run(&project, &env, &["doctor"], 0));
    assert!(doctor.contains("nl_role_triggers=DEFERRED_OPENCODE_UNVALIDATED"));
    assert!(doctor.contains("INFO nl_role_triggers=DEFERRED_OPENCODE_UNVALIDATED"));
    assert!(!doctor.contains("PASS nl_role_triggers=PASS"));
