...
agent 应把 `刷新` / `refresh` 优先理解为 AiPlus refresh，并先回复当前 Auto
Compact、Auto Team Consultant 和 compact-state 状态，再继续工作。
...
v0.1.1 的 one-command installer 先验证 macOS Apple Silicon。其它平台在 release
...
Codex、Claude Code 或 OpenCode config。AiPlus v0.1.1 先发布已验证的 macOS Apple
...
global config edit、telemetry、auto-update 或 runtime network fetch。v0.1.1
