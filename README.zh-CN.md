...
### Profile Supplemental Bundle（补充包）

AiPlus 支持在 private profile source 中放置可选的**补充包文件**。当这些文件存在时，
`aiplus profile install` 会安全地把它们复制到已安装的 profile 目录：

- `USER.md` — Owner 快照和稳定偏好
- `MEMORY.md` — Profile 级别的记忆快照
- `preferences/` — 偏好分类文件
- `identities/` — 角色身份定义
- `sync/` — 同步策略和项目映射文件

这些文件永远不会被复制到 public release 资产中。Context 命令
（`aiplus profile context`、`aiplus user context`）会从已安装 profile 读取，
并对看起来像 secret 或 API key 的行进行脱敏。
...
式使用 `--aliases`。Bitwarden value 如果是 empty、whitespace-only 或
`PENDING_OWNER_INPUT_DO_NOT_USE`，AiPlus 会视为未配置，并且不会注入。
...
AiPlus v0.5.1 把 public `aiplus-agent-memory` 接入了 `aiplus refresh`、
`aiplus status` 和 `aiplus doctor`。这些命令会报告 memory records、role
identities、skill candidates、private profile 是否存在、`secret_values=none` 和
`global_agent_config=untouched`。

v0.5.0 新增的 `aiplus-agent-memory` foundation 用于临时 terminal agent 的本地连续性。
它把 Memory Context、Role Identity 和 Skill Candidate 存在当前项目的 `.aiplus/` 下。
...
安装后的 Codex、Claude Code、OpenCode 项目级 guidance 会把自然语言映射到这些命令：

- “记住这个”/“记住这个偏好”：先脱敏，再写 project memory。
- “以后都这样”：只创建 profile/global candidate，不自动批准全局规则。
- “只在这个项目用”：写 project memory。
- “忘掉这个”：运行 `aiplus memory forget <id>`；不明确时先问是哪条 memory。
- “你记住了什么”/“这次用了哪些记忆”：运行 memory status/context。
- “新开顾问”/“新开 advisor”：加载 advisor Role Identity。
- “新开 CEO”：加载 CEO Role Identity。
...
- **AiPlus Auto Compact** (`auto-compact`)：主动 compact reminder、checkpoint、
  handoff、savings preview、validate 和 resume workflow assets。
...
AiPlus Auto Compact 不替代 host compact 按钮或 `/compact`。它的核心价值是主动
提醒时机：agent 可以在稳定且高价值的时机调用 `aiplus compact remind`，先准备
checkpoint，估算 token/USD savings，然后决定 wait、prepare_only，还是建议用户手动
host compact。
...
Reminder 会输出 `REMINDER_DECISION`、`REMINDER_LEVEL`、`HANDOFF_STATE`、
`RECOVERY_CONFIDENCE`、token/USD savings 估算和 `SECRET_VALUES_PRINTED=no`。
只有 `remind_now` 才表示可以在 prepare/checkpoint 后建议用户手动 host compact；
`prepare_only`、`wait`、`blocked` 都表示还不应建议 compact。
