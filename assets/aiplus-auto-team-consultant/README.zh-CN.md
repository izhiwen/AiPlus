...
Reviewer findings、Builder changed files、Owner gates 和 next safe action。
 ...
 proactive compact 管理方面，agent 可以在安全且高价值的时机主动运行
`aiplus compact remind`（HEAVY 任务每 30 分钟或阶段边界，MEDIUM 任务在阶段边界或
review/QA 前）。`aiplus compact checkpoint` 可以在创建 checkpoint 前验证 readiness。
v2.1 中，`aiplus compact prepare` 还会构建 context capsule
（`.codex/compact/context-capsule.json`），供 `aiplus compact resume` 读取以实现更
丰富的 session 恢复。
...
`aiplus profile context`、`aiplus secret-broker status` 或
`aiplus secret-broker doctor`。如需更深诊断，`aiplus profile doctor` 验证 profile
bundle 完整性和 identity files；`aiplus memory doctor` 扫描 stale 或冲突的 memory
records。Auto Team Consultant 可以使用 user-level profile，但优先级必须低于当前
Owner message 和项目规则，而且不能把 private profile material 复制到 public docs、
task packets、compact files 或 result packets。
